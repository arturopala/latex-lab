...
trait DocumentService {
  ...
  def setDocumentFile(documentId: String, file: File, charset: Charset): Future[Try[String]]
}
...
class DocumentServiceActorStage(workspace: Workspace, actorSystem: ActorSystem, timeout: FiniteDuration) extends DocumentService {
  ...
  def setDocumentFile(documentId: String, file: File, charset: Charset): Future[Try[String]] = (router ? SetDocument(documentId, file, charset)).asInstanceOf[Future[Try[String]]]
}